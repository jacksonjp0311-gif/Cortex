...
class TeachSeedTests(unittest.TestCase):
    def test_memory_packets_load_and_seed(self) -> None:
        from cortex.teach_seed import claims_to_memories, load_memory_packets, seed_intelligence
        ...
        self.assertTrue(
            cons.get("created") or cons.get("status") in {"created", "duplicate_skip"}
        )
        store.close()
...
class ProgressStackTests(unittest.TestCase):
    def test_profiles_and_control_error(self) -> None:
        ...
        self.assertTrue(agent["block"])
        self.assertEqual(agent["immune_action"]["code"], "STOP_NO_HOST_MUTATION")
        self.assertTrue(agent["read_first"])
        self.assertTrue(minimal["block"])
        self.assertTrue(debug["block"])
