"""Alpha.34 four-call live structured-edit repair screen."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not private:
        raise ValueError("alpha.34 external-private evaluator bundle is unavailable")
    plan = {
        "schema_version": "cortex-alpha34-live-structured-repair-screen/1.0",
        "state": "STRUCTURED_REPAIR_SCREEN_FROZEN",
    ...
    try:
        ...
        if provenance.get("evidence_class") != EVIDENCE_LIVE:
            ...
            principal_id = f"alpha34-{int(time.time())}-{secrets.token_hex(4)}"
            principal_secret = secrets.token_urlsafe(32)
            register_will_principal(store, args.repo, principal_id, "Alpha.34 structured repair operator", secret=principal_secret)
        ...
        grant = CapabilityGrant(
            ...
            allowed_tools=(),
            principal_id="alpha34_live_screen",
            purpose="exact four-call task-only structured repair screen",
            issued_at=now,
            expires_at=now + 1800,
            max_tool_calls=0,
        ...
        report = {
            **plan,
            "state": result["status"] if audit["valid"] else "STRUCTURED_REPAIR_SCREEN_HELD",
