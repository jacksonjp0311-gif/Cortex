"""Alpha.35 same-model harder external-private structured repair screen."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not private:
        raise ValueError("alpha.35 external-private evaluator bundle is unavailable")
    plan = {
        "schema_version": "cortex-alpha35-live-harder-structured-repair-screen/1.0",
        "state": "HARDER_STRUCTURED_REPAIR_SCREEN_FROZEN",
    ...
    try:
        ...
        if provenance.get("evidence_class") != EVIDENCE_LIVE:
            ...
            principal_id = f"alpha35-{int(time.time())}-{secrets.token_hex(4)}"
            principal_secret = secrets.token_urlsafe(32)
            register_will_principal(store, args.repo, principal_id, "Alpha.35 harder repair operator", secret=principal_secret)
        ...
        grant = CapabilityGrant(
            ...
            allowed_tools=(),
            principal_id="alpha35_live_screen",
            purpose="exact four-call harder structured repair screen",
            issued_at=now,
            expires_at=now + 1800,
            max_tool_calls=0,
        ...
        report = {
            **plan,
            "state": "HARDER_STRUCTURED_REPAIR_SCREEN_RECONSTRUCTED" if audit["valid"] else "HARDER_STRUCTURED_REPAIR_SCREEN_HELD",
